  ...
  for (int i = 0; i < dxf.paths.count(); i++) {
    ...
    if (first_open_path) {
      PRINTF("WARING: Open paths in dxf_liniear_extrude(file = \"%s\", layer = \"%s\"):",
              filename.toLatin1().data(), layername.toLatin1().data());
...
QString DxfLinearExtrudeNode::dump(QString indent) const {
  if (dump_cache.isEmpty()) {
    ...
    stat(filename.toLatin1().data(), &st);
    text.sprintf("dxf_linear_extrude(file = \"%s\", cache = \"%x.%x\", layer = \"%s\", "
            "height = %f, origin = [ %f %f ], scale = %f, center = %s",
            filename.toLatin1().data(), (int) st.st_mtime, (int) st.st_size,
            layername.toLatin1().data(), height, origin_x, origin_y, scale,
            center ? "true" : "false");
    if (has_twist) {
      QString t2;
